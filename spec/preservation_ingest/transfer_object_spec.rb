describe Robots::SdrRepo::PreservationIngest::TransferObject do
  ...
  let(:vm_file_path) do
    input_dir = File.join(Settings.transfer_object.input_dir, bare_druid)
  ...
  it 'raises ItemError if versionMetadata.xml file for druid does not exist' do
    ...
    cmd_regex = Regexp.new(".*ssh #{Settings.transfer_object.dor_host} test -e #{vm_file_path}.*")
  ...
  it 'executes the tarpipe command to transfer the object when no errors are raised' do
    ...
    cmd_regex = Regexp.new("if ssh #{Settings.transfer_object.dor_host} test -e #{vm_file_path}.*")
